...
const refreshProgress = async () => {
  try {
    // If we have fileScraperId, check active job state first
    if (fileScraperId) {
      const activeRes = await fetch(`/api/scraper/file/${fileScraperId}/active-job`);
      if (activeRes.ok) {
        const activeInfo = await activeRes.json();
        // If running by another user, show blocked screen and keep polling
...
// ==============================================================================
// Stop Button Handler (Owner only)
// ==============================================================================
