(function(){
  ...
  /* ---------- Ultra-Smooth FAQ Accordion Controller ---------- */
  ...
  SmoothAccordion.prototype.onClick = function(e) {
    e.preventDefault();
  ...
  SmoothAccordion.prototype.shrink = function() {
    this.isClosing = true;
    ...
    this.animation.oncancel = function() {
      ...
      self.el.classList.remove('is-closing');
    };
  };

  SmoothAccordion.prototype.open = function() {
    var container = this.el.closest('.faq, .dynamic-faq-block, [id^="faq"]') || this.el.parentElement;
    if (container) {
      var allDetails = container.querySelectorAll('details');
      var self = this;
      allDetails.forEach(function(other) {
        if (other !== self.el && other.open) {
          if (other._accordionController) {
            other._accordionController.shrink();
          } else {
  ...
  SmoothAccordion.prototype.onAnimationFinish = function(isOpen) {
    this.el.open = isOpen;
    this.animation = null;
    this.isClosing = false;
    this.isExpanding = false;
  ...
  function initFaqAccordion() {
    ...
    faqContainers.forEach(function(container) {
      ...
      allDetails.forEach(function(detail) {
        if (!detail._accordionController) {
          new SmoothAccordion(detail);
        }
      });
    });
  }
