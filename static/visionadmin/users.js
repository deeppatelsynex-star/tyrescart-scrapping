...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function showToast(message, type = 'success') {
    ...
    toast.className = `fixed bottom-6 right-6 z-50 px-5 py-3 rounded-2xl shadow-xl border text-xs font-bold flex items-center gap-2.5 transition-all duration-300 transform translate-y-10 opacity-0 ${
      type === 'success' 
        ? 'bg-[#EFF6FF] border-[#BFDBFE] text-[#2E7D32]' 
    ...
    tableBody.innerHTML = filtered.map(u => {
      ...
      if (isTrash) {
        ...
        statusBadge = u.is_active 
          ? `<button type="button" class="btn-toggle-status px-2.5 py-1 rounded-full text-[11px] font-bold bg-[#EFF6FF] text-[#2E7D32] border border-[#BFDBFE] hover:bg-blue-100 transition flex items-center gap-1.5 cursor-pointer" data-id="${u.id}" title="Click to toggle status"><span class="w-2 h-2 rounded-full bg-[#00A650] animate-pulse"></span>Active</button>`
          ...
          <td class="py-4 px-6 text-right">
            ${isTrash ? `
              <div class="flex items-center justify-end gap-1.5">
                <button type="button" class="btn-restore-user p-2 rounded-xl text-[#2563FF] hover:text-blue-800 hover:bg-blue-50 transition cursor-pointer" data-id="${u.id}" data-name="${escapeHtml(u.name)}" title="Restore Account">
            ...
            ` : `
              <div class="flex items-center justify-end gap-1.5">
                <button type="button" class="btn-edit-user p-2 rounded-xl text-slate-500 hover:text-[#00A650] hover:bg-[#EFF6FF] transition cursor-pointer" data-id="${u.id}" title="Edit Administrator">
  ...
  if (tabBtnActive) {
    tabBtnActive.addEventListener('click', () => {
      ...
      currentView = 'active';
      tabBtnActive.className = 'tab-user-view px-4 py-2 rounded-xl bg-[#EFF6FF] text-[#1D4ED8] shadow-2xs transition cursor-pointer';
